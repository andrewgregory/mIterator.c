...
int main(void) {
  char ctx[] = "ctx";
  mit_t *mit = mit_finite_new(NULL, ctx, freefn);

  tap_plan(8);

  tap_ok(mit != NULL, "mit_finite_new returns a valid pointer");
  ...
  tap_is_int(mit_is_finite(mit), 1, "iterator indicates termination");
